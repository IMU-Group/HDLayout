...
def main(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        wandb.log({'train': train_stats, 'test': test_stats})

        render = DataRender()
        res_render = render.render(render_res)
        wandb.log({
            "input_imgs": [wandb.Image(pil, caption=f'input_{epoch:02d}_{i:02d}.png')
                            for i, pil in enumerate(res_render['input_imgs'])],
            "block_imgs": [wandb.Image(pil, caption=f'block_{epoch:02d}_{i:02d}.png')
                            for i, pil in enumerate(res_render['block_bbox'])],
            "line1_imgs": [wandb.Image(pil, caption=f'line1_{epoch:02d}_{i:02d}.png')
                            for i, pil in enumerate(res_render['line1_bbox'])],
            "line2_imgs": [wandb.Image(pil, caption=f'line2_{epoch:02d}_{i:02d}.png')
                                    for i, pil in enumerate(res_render['line2_bezier'])],
        })
